"""
SynthFix training: router-gated REINFORCE with split-symbolic reward.
...
if 'torch._dynamo' not in _sys.modules:
    # Prefer the *real* torch._dynamo when it imports cleanly (torch>=2.5,
    # needed by flex_attention on modern models like Qwen3). Only fall back
    # to a stub on environments where importing it fails (older torch).
    try:
...
from models.symbolic import (classify_token_string, compute_reward_split,
                              ...
                              compute_reward_split_rich,
                              compute_reward_from_split_rich,
                              compute_reward_split_rich_exec)
...
MODEL_PATHS = {
    ...
    # Newest model (Qwen3, 2025); needs transformers>=4.51 — run via the
    # isolated env .venv_qwen35 (see artifact/queue_artifact.py).
    'qwen3-4b': 'Qwen/Qwen3-4B-Base',
}

LORA_TARGET_MODULES = {
    'deepseek-1.3b': ['q_proj', 'k_proj', 'v_proj', 'o_proj'],
    'llama3.2-3b': ['q_proj', 'k_proj', 'v_proj', 'o_proj'],
    'qwen2.5-coder-1.5b': ['q_proj', 'k_proj', 'v_proj', 'o_proj'],
    ...
    'qwen3-4b': ['q_proj', 'k_proj', 'v_proj', 'o_proj'],
}
...
def _detect_language(dataset_path: str) -> str:
    ...
    if 'codeflaws' in p or '/c/' in p:
        return 'c'
    if 'sven' in p or '/python/' in p or 'pyrepair' in p or 'mbpp' in p \
            or 'quixbugs' in p:
        return 'python'
    return 'javascript'
...
# Module-level default; overridable via args.rl_beta for controlled sweeps.
BETA_RL = 0.25
# KL penalty against the SFT reference (adapter-disabled
# forward pass).  Standard RLHF anchor — without it the RL gradient
# can drift the policy off the SFT ridge even with a moderate beta_rl,
# which can reduce greedy validation performance.
# Default is off (0.0); set via args.kl_beta for KL-anchored runs.
BETA_KL = 0.0
...
RLOO_K = 2
# Temperature for REINFORCE continuation sampling. The default encourages
# candidates that differ from greedy decoding while staying near the policy.
RL_SAMPLE_TEMP = 0.9
RL_TOP_P = 0.95
...
# Rich reward: chrF carries most of the weight as a surface-similarity
# anchor; parse, data-flow, and lint signals contribute additional
# symbolic evidence without dominating the objective.
RICH_REWARD_WEIGHTS = dict(
    lambda_ast=0.05, lambda_cfg=0.10, lambda_sem=0.00,
    lambda_sim=0.55,
    lambda_parse=0.08, lambda_dfg=0.12, lambda_lint=0.10,
)

# Rich+exec reward (Codeflaws). Execution pass rate gets substantial
# weight (0.35) because it's the only *functional* ground-truth signal
# we have — a patch that compiles and passes all tests is by
# definition correct, regardless of surface similarity. chrF stays at
# 0.30 as a proxy for CodeBLEU surface so the gradient remains well-
# defined when tests can't be run (test_dir missing). Remaining 0.35
# is split over the static symbolic signals.
RICH_EXEC_REWARD_WEIGHTS = dict(
    lambda_ast=0.03, lambda_cfg=0.07, lambda_sem=0.00,
    lambda_sim=0.25,
    lambda_parse=0.06, lambda_dfg=0.06, lambda_lint=0.05,
    lambda_exec=0.35,
    lambda_repair_effect=0.13,
)
...
def train_synthfix(model, tokenizer, router, train_loader, val_loader,
    ...
    beta_kl    = float(getattr(args, 'kl_beta',  BETA_KL))
    ...
    use_rich_reward = bool(getattr(args, 'use_rich_reward', False))
    use_rich_exec_reward = bool(getattr(args, 'use_rich_exec_reward', False))
    if use_rich_exec_reward:
        # exec implies rich
        use_rich_reward = True

    # --- Ablation switches (RQ2 router, RQ5 reward components) ---
    # no_router: disable the adaptive router; apply the RFT update uniformly to
    #   every sample in phase 2 (i.e. a non-adaptive fixed-schedule hybrid).
    no_router = bool(getattr(args, 'no_router', False))
    # ablate_reward: drop a group of symbolic reward components and renormalize
    #   the remaining weights so the reward stays on the same scale.
    ablate_reward = getattr(args, 'ablate_reward', 'none') or 'none'
    exec_weights = dict(RICH_EXEC_REWARD_WEIGHTS)
    rich_weights = dict(RICH_REWARD_WEIGHTS)
    _ablate_groups = {
        'no_exec': ['lambda_exec'],
        'no_struct': ['lambda_ast', 'lambda_cfg', 'lambda_dfg'],
        'no_repair': ['lambda_repair_effect'],
        'no_lint': ['lambda_lint'],
    }
    if ablate_reward != 'none':
        for _k in _ablate_groups.get(ablate_reward, []):
            exec_weights[_k] = 0.0
            rich_weights[_k] = 0.0
        _se = sum(exec_weights.values())
        if _se > 0:
            exec_weights = {k: v / _se for k, v in exec_weights.items()}
        _sr = sum(rich_weights.values())
        if _sr > 0:
            rich_weights = {k: v / _sr for k, v in rich_weights.items()}
    _log(f'  Ablations: no_router={no_router} ablate_reward={ablate_reward}')
    ...
    # on generated candidates. Split-symbolic signals are used in the
    # reward function and at inference time.
    ...
    _log(f'    loss = sft_ce + {beta_rl} * E[hard_gate * adv_LOO * CE_sampled] '
         f'+ {beta_kl} * KL(pi || pi_SFT)  (RLOO K={rloo_k})')
    if use_rich_exec_reward:
        _log(f'    reward (RICH+EXEC) = '
             f'{RICH_EXEC_REWARD_WEIGHTS["lambda_sim"]}*r_SIM + '
             f'{RICH_EXEC_REWARD_WEIGHTS["lambda_exec"]}*r_EXEC + '
             f'{RICH_EXEC_REWARD_WEIGHTS["lambda_repair_effect"]}*r_REPAIR_EFFECT + '
             f'{RICH_EXEC_REWARD_WEIGHTS["lambda_parse"]}*r_PARSE + '
             f'{RICH_EXEC_REWARD_WEIGHTS["lambda_dfg"]}*r_DFG + '
             f'{RICH_EXEC_REWARD_WEIGHTS["lambda_lint"]}*r_LINT + '
             f'{RICH_EXEC_REWARD_WEIGHTS["lambda_cfg"]}*r_CFG + '
             f'{RICH_EXEC_REWARD_WEIGHTS["lambda_ast"]}*r_AST')
    elif use_rich_reward:
        _log(f'    reward (RICH) = '
             f'{RICH_REWARD_WEIGHTS["lambda_ast"]}*r_AST + '
             f'{RICH_REWARD_WEIGHTS["lambda_cfg"]}*r_CFG + '
             f'{RICH_REWARD_WEIGHTS["lambda_sem"]}*r_SEM + '
             f'{RICH_REWARD_WEIGHTS["lambda_sim"]}*r_SIM + '
             f'{RICH_REWARD_WEIGHTS["lambda_parse"]}*r_PARSE + '
             f'{RICH_REWARD_WEIGHTS["lambda_dfg"]}*r_DFG + '
             f'{RICH_REWARD_WEIGHTS["lambda_lint"]}*r_LINT')
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for batch in progress:
            ...
            if not is_curriculum:
                ...
                # test_dirs is present in the Codeflaws loader; falls
                # back to empty strings for JS/Python datasets.
                test_dirs = batch.get('test_dirs',
                                       [''] * len(fixed_texts))
                ...
                for _ in range(rloo_k):
                    ...
                    for j in range(g.size(0)):
                        ...
                        if use_rich_exec_reward:
                            rs = compute_reward_split_rich_exec(
                                cont, fixed_texts[j], lang=lang,
                                test_dir=test_dirs[j],
                                buggy=buggy_texts[j],
                                max_tests=5)
                            r = compute_reward_from_split_rich(
                                rs, **exec_weights)
                        elif use_rich_reward:
                            rs = compute_reward_split_rich(
                                cont, fixed_texts[j], lang=lang)
                            r = compute_reward_from_split_rich(
                                rs, **rich_weights)
                        else:
                            rs = compute_reward_split(cont, fixed_texts[j])
                            r = compute_reward_from_split(
                                rs, **REWARD_WEIGHTS)
                ...
                with torch.no_grad():
                    ...
                    if no_router:
                        # Non-adaptive fixed-schedule hybrid: route every
                        # sample through the RFT pathway in phase 2.
                        hard_gate = torch.ones_like(prob_hard)
                    else:
                ...
                def _rl_backward(gen_ids_k, adv_k):
                    ...
                    # KL anchor to SFT reference (the LoRA base without
                    # adapter).  per_token KL ~= logp_policy - logp_ref,
                    # estimated on the generated tokens.  logp_policy is
                    # -per_tok (already grad-enabled); logp_ref is
                    # computed without the adapter and w/o grad.
                    total_loss_k = rl_per
                    if beta_kl > 0.0:
                        with torch.no_grad():
                            try:
                        ...
                        if ref_out is not None:
                            ref_per_tok = F.cross_entropy(
                                ref_out.logits[..., :-1, :]
                                    .contiguous().view(-1, vocab),
                                shift_labels.view(-1),
                                reduction='none', ignore_index=-100,
                            ).view(bs_, sl_m1)
                            # per-sample seq-mean KL (k1 estimator),
                            # clamped at 0 so it's always a penalty.
                            kl_per_tok = (ref_per_tok - per_tok)
                            per_sample_kl = (kl_per_tok * tok_mask).sum(1) \
                                / n_tok.clamp(min=1)
                            per_sample_kl = per_sample_kl.clamp(min=0.0)
                            total_loss_k = rl_per + beta_kl * per_sample_kl
                            del ref_out, ref_per_tok, kl_per_tok

                    # Divide by rloo_k so the gradient magnitude is
                    # comparable to the single-sample loss.
                    rl_loss_k = (hard_gate * total_loss_k * valid_k).sum() \
                    ...
                    del per_sample_ce_k, rl_loss_k, total_loss_k
                    return val_k
        ...
        # Selection criterion. Default 'val_loss' matches the SFT/RFT
        # baselines for apples-to-apples. 'val_codebleu' selects the epoch
        # with the best val generation quality — used for exec-reward RL
        # runs so the saved policy is the post-RL model when RL actually
        # improves generations, and falls back to the (SFT-equivalent)
        # warmup epoch otherwise. This guarantees greedy never regresses
        # below the SFT warmup while capturing any RL upside. Leak-free:
        # selection only ever consults the validation split.
        select_metric = getattr(args, 'select_metric', 'val_loss')
        if select_metric == 'val_codebleu':
            improved = val_cb > best_val_cb
        else:
