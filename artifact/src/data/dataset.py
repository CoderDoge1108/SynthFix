...
class RepairDataset(Dataset):
    ...
    Also supports the text-pair format (split_buggy.txt / split_fixed.txt).
    ...
    def __init__(self, data_dir, split='train', max_length=512):
        ...
        if json_file.exists():
            ...
            # Optional per-sample test directory (Codeflaws: points to
            # raw bug dir with input-pos* / output-pos* test cases).
            self.test_dirs = [item.get('test_dir', '') for item in data]
        else:
            # Fall back to text-pair format.
            ...
            self.test_dirs = [''] * len(self.buggy_codes)
    ...
    def __getitem__(self, idx):
        return {
            ...
            'index': idx,
            'test_dir': self.test_dirs[idx]
                if idx < len(self.test_dirs) else '',
        }
...
def collate_fn(batch, tokenizer, max_length=512):
    ...
    # Optional per-sample test directory (Codeflaws). Carried through so
    # downstream inference can run execution-based reward.
    test_dirs = [item.get('test_dir', '') for item in batch]
