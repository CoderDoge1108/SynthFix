...
try:
    from .parse_dfg import parse_score as _ts_parse, dfg_score as _ts_dfg
    _HAS_PARSE_DFG = True
except Exception:
    _HAS_PARSE_DFG = False

try:
    from .lint_reward import lint_score as _lint_score
    _HAS_LINT = True
except Exception:
    _HAS_LINT = False
...
def generate_k_candidates(model, tokenizer, prompt_ids, prompt_mask,
    ...
    Candidate layout (wider-temperature schedule for real diversity):
      ...
      K=32 : [greedy, T=0.3 x4, T=0.6 x6, T=0.9 x8,
              T=1.2 x6, T=1.5 x6, identity]
    """
    ...
    for kind, temp, nrs in slots:
        ...
        texts, logp_avg = _decode_with_logp(
            seqs, scores, prompt_pad_len, pad_id, tokenizer, bs)
...
FEATURE_NAMES = [
    ...
    # SynthFix-Exec extensions — tree-sitter-derived symbolic features.
    'ts_parse', 'ts_dfg_to_buggy',
    # Execution-based feature (Codeflaws): fraction of test cases passed.
    # 0.5 is the neutral default when no test_dir is available.
    'exec_pass_rate',
    # Language-specific static-linter reward in [0, 1].
    'lint_quality',
    # Reference-free local repair-effect score vs buggy code.
    'repair_effect',
]
NUM_FEATURES = len(FEATURE_NAMES)
# Named column offsets — keep in sync with FEATURE_NAMES above.
COL_PARSE = 13
COL_DFG = 14
COL_EXEC = 15
COL_LINT = 16
...
# Per-inference language override. Set via set_feature_language() so
# extract_features knows which tree-sitter grammar to use.
_FEAT_LANG = {'lang': 'javascript'}


def set_feature_language(lang: str) -> None:
    """Pick the tree-sitter grammar used for ts_parse / ts_dfg features."""
    if lang in ('python', 'javascript', 'c'):
        _FEAT_LANG['lang'] = lang
...
def extract_features(buggy: str,
                      ...
                      is_identity_flags: List[bool],
                      test_dir: Optional[str] = None) -> np.ndarray:
    ...
    When ``test_dir`` is provided (Codeflaws only), an execution-based
    ``exec_pass_rate`` feature is computed by compiling each candidate
    and running it against the per-bug test suite.
    """
    K = len(candidates)
    ...
    for i in range(K):
        ...
        # SynthFix-Exec additions — real AST parseability and DFG signal.
        lang = _FEAT_LANG['lang']
        if _HAS_PARSE_DFG:
            try:
                feats[i, COL_PARSE] = _ts_parse(ca, lang)
                feats[i, COL_DFG] = _ts_dfg(ca, buggy, lang)
            except Exception:
                feats[i, COL_PARSE] = 0.5
                feats[i, COL_DFG] = 0.5
        else:
            feats[i, COL_PARSE] = ast_scores[i]
            feats[i, COL_DFG] = 0.5
        # exec_pass_rate — filled below when test_dir is given.
        feats[i, COL_EXEC] = 0.5
        # Lint reward — real static-analysis signal (pyflakes / gcc /
        # tree-sitter-based undefined-ref).
        if _HAS_LINT:
            try:
                feats[i, COL_LINT] = _lint_score(ca, lang)
            except Exception:
                feats[i, COL_LINT] = 0.5
        else:
            feats[i, COL_LINT] = 0.5
        try:
            from .repair_effect import repair_effect_score
            feats[i, COL_REPAIR] = repair_effect_score(ca, buggy, lang)
        except Exception:
            feats[i, COL_REPAIR] = 0.5

    # Optional execution-based feature (Codeflaws).
    if test_dir and _HAS_PARSE_DFG:
        try:
            from .exec_reward import exec_reward
            bd = Path(test_dir)
            if bd.exists():
                for i, ca in enumerate(candidates):
                    # Identity candidates are pre-filtered at eval; they
                    # remain in validation labeling for consistent features.
                    r = exec_reward(ca, bd, max_tests=5)
                    feats[i, COL_EXEC] = float(r.pass_rate)
        except Exception:
            pass
...
class LearnedReranker:
    ...
    model predictions are near-constant, fall back to a
