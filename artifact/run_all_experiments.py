...
Uses available GPUs in parallel where possible.

Default artifact hyperparameters:
  - lr=2e-4, epochs=4, LoRA r=16, alpha=32
...
if 'torch._dynamo' not in sys.modules:
    ...
    # needed by flex_attention on modern models like Qwen3). Stub only as a
    # fallback for older-torch environments where the import fails.
    try:
...
# All models use the artifact config: lr=2e-4, epochs=4, LoRA r=16
...
def run_worker(args):
    ...
    # policy stays close to the teacher.
        ...
        if getattr(args, 'init_from_ckpt', None):
            ...
            warmup_epochs = min(args.sft_warmup_epochs, epochs)
            P(f'  [{tag}] SFT warmup = {warmup_epochs} epoch(s) '
              f'(then {epochs - warmup_epochs} REINFORCE epoch(s))')
        else:
        ...
        # Persist the final RFT adapter so it can be evaluated later
        # (functional pass@1 / security). Without this the RFT checkpoint
        # is lost in /tmp.
        if getattr(args, 'save_ckpt_to', None):
            os.makedirs(args.save_ckpt_to, exist_ok=True)
            model.save_pretrained(args.save_ckpt_to)
            P(f'  [{tag}] Saved final RFT ckpt -> {args.save_ckpt_to}')
    ...
    # ── SynthFix (per-sample routing) ──────────────────────────────
        ...
        for _k in ('rl_beta', 'kl_beta', 'rloo_k', 'rl_temp', 'rl_top_p',
                   'rl_no_repeat_ngram'):
            _v = getattr(args, _k, None)
            if _v is not None:
                setattr(_a, _k, _v)
                P(f'  [{tag}] override {_k}={_v}')
        # Rich reward adds r_parse + r_dfg + r_lint to the RL reward.
        if getattr(args, 'use_rich_reward', False):
            _a.use_rich_reward = True
            P(f'  [{tag}] enabling RICH reward (parse + dfg + lint)')
        if getattr(args, 'use_rich_exec_reward', False):
            _a.use_rich_exec_reward = True
            P(f'  [{tag}] enabling RICH+EXEC reward '
              f'(rich + compiled test-case pass rate)')
        # Two-stage: if we start from an SFT ckpt, skip the SynthFix
        # warmup phase and go straight to router-gated RLOO.
        if getattr(args, 'init_from_ckpt', None):
        ...
        _a.select_metric = getattr(args, 'select_metric', 'val_loss')
        if _a.select_metric != 'val_loss':
            P(f'  [{tag}] model selection by {_a.select_metric}')
        # Ablation switches (RQ2 router, RQ5 reward components).
        _a.no_router = bool(getattr(args, 'no_router', False))
        _a.ablate_reward = getattr(args, 'ablate_reward', 'none')
        if _a.no_router:
            P(f'  [{tag}] ABLATION: router disabled (fixed-schedule hybrid)')
        if _a.ablate_reward and _a.ablate_reward != 'none':
            P(f'  [{tag}] ABLATION: reward={_a.ablate_reward}')

        # Persistent checkpoint dir for faster iteration. Allow artifact
        # runs to pin the output under artifact/checkpoints instead of
        # ephemeral /tmp so results remain reproducible after reboot.
        if getattr(args, 'save_ckpt_to', None):
            ckpt_dir = args.save_ckpt_to
        else:
    ...
    # This continuation-only protocol avoids prompt-echo inflation from
    # evaluating the entire prompt+completion string.
    #
    ...
    # all_reranked_noid excludes the identity candidate (= buggy code)
    # from reranker-argmax. This prevents a surface-similarity label from
    # favoring the unchanged input on minimal-edit examples.
    all_greedy = []
    all_reranked_noid = []
        ...
        for batch in test_loader:
            ...
            if method != 'synthfix' or NO_RERANK:
                ...
                for j in range(cur_bs):
                    ...
                    # Configurable margin for preserving the greedy floor
                    # unless the reranker assigns a sufficiently higher
                    # score to a sampled candidate.
                    ...
                    # No-identity diagnostic: exclude unchanged-input
                    # candidates before argmax.
                    is_id = cand_feats[:, 10]
                    if (is_id < 0.5).any():
                        masked = np.where(is_id < 0.5, scores, -np.inf)
                        best_ni = int(np.argmax(masked))
                        max_score_ni = float(masked[best_ni])
                        if (max_score_ni - scores[greedy_idx]) < RERANK_MARGIN:
                            best_ni = greedy_idx
                    else:
                        best_ni = greedy_idx
                    all_reranked_noid.append(cands[best_ni])
    ...
    if method == 'synthfix' and all_greedy:
        ...
        cb_rerank_noid = _compute_codebleu(
            all_reranked_noid, all_ref, lang=lang)
        em_noid = sum(1 for g, r in zip(all_reranked_noid, all_ref)
                       if g == r)
        P(f'  [{tag}] greedy={cb_greedy*100:.2f}%  '
          f'rerank={cb*100:.2f}%  '
          f'rerank_noid={cb_rerank_noid*100:.2f}% (EM_noid={em_noid})  '
          f'delta_rerank={(cb-cb_greedy)*100:+.2f}pp  '
          f'delta_noid={(cb_rerank_noid-cb_greedy)*100:+.2f}pp')
    ...
    if method == 'synthfix':
        ...
        if cb_rerank_noid is not None:
            results['codebleu_rerank_noid'] = cb_rerank_noid
            results['reranker_delta_pp_noid'] = (
                (cb_rerank_noid - cb_greedy) * 100)
...
def run_experiment(model_cfg, dataset_name, data_dir, exp_dir):
    """Single-experiment runner kept for backwards compatibility.
...
def main():
    ...
    _log('=' * 70)
    _log('  SynthFix: FULL EXPERIMENT SUITE (artifact config)')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_ckpt_to', type=str, default=None,
                        help='Save the best LoRA ckpt to this path '
                             '(two-stage training).')
    parser.add_argument('--select_metric', type=str, default='val_loss',
                        choices=['val_loss', 'val_codebleu'],
                        help='SynthFix model-selection criterion. '
                             'val_codebleu keeps the best-generating epoch '
                             '(post-RL when RL helps; SFT-warmup otherwise).')
    ...
    parser.add_argument('--kl_beta', type=float, default=None,
                        help='KL-anchor weight to SFT reference '
                             '(0 = off, 0.05-0.15 recommended for rich).')
    ...
    parser.add_argument('--use_rich_reward', action='store_true',
                        help='Use the SynthFix-Exec rich reward '
                             '(adds r_parse + r_dfg + r_lint to the '
                             'RL reward mixture).')
    parser.add_argument('--use_rich_exec_reward', action='store_true',
                        help='Use RICH + execution-based test-case '
                             'reward (Codeflaws). Requires batch to '
                             'carry test_dirs; falls back to rich '
                             'elsewhere.')
    parser.add_argument('--no_router', action='store_true',
                        help='RQ2 ablation: disable the adaptive router '
                             '(non-adaptive fixed-schedule hybrid).')
    parser.add_argument('--ablate_reward', type=str, default='none',
                        choices=['none', 'no_exec', 'no_struct',
                                 'no_repair', 'no_lint'],
                        help='RQ5 ablation: drop a symbolic reward '
                             'component group and renormalize.')
